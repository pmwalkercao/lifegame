...
public class GameFrame extends JFrame {
  ...
  public GameFrame() {
    ...
    game = new Game(width, height, 1000).init();  //将所有格子初始化为DEATH
        ...
        btn.addActionListener((e) -> {
          ...
          if (!run) {
            if (btn.getBackground() == CellButton.DEATH_COLOR) {
              ...
              game.getCellByXY(btn.getxInCellPanel(), btn.getyInCellPanel())
                .setStatus(Cell.LifeStatus.SURVIVAL);
            } else {
              btn.setBackground(CellButton.DEATH_COLOR);
              game.getCellByXY(btn.getxInCellPanel(), btn.getyInCellPanel())
                .setStatus(Cell.LifeStatus.DEATH);
            }
          }
        });
        leftPanel.add(btn);
